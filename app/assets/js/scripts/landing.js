...
function updateSelectedServer(serv){
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.rawServer.name : '서버가 선택되지 않음')
    if(getCurrentView() === VIEWS.settings){
...
const refreshMojangStatuses = async function(){
    loggerLanding.info('Mojang 서비스 상태 새로고침 중..')
    ...
    if(jvmDetails == null) {
        ...
        setOverlayHandler(() => {
            ...
            try {
                ...
                showLaunchFailure('자바를 다운로드 하는 중 오류가 발생했습니다.', '콘솔을 열어 (CTRL + Shift + i) 자세한 정보를 확인하세요.')
            }
        })
        setDismissHandler(() => {
