        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>계정 선택됨 &#10004;' : '>계정 선택'}</button>
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `삭제하는 데에 실패했습니다.<br>드래그하여 모드 추가 ${fullName}`,
...
async function populateJavaExecDetails(execPath){
    ...
    if(details != null) {
        ...
        settingsJavaExecDetails.innerHTML = '알 수 없는 버전'
    }
}
...
function populateJvmOptsLink(server) {
    ...
    settingsJvmOptsLink.innerHTML = `Java ${major}(HotSpot VM)에 사용 가능한 옵션`
    if(major >= 12) {
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = '사전 릴리스'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = 'Stable 버전'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/gyeongent/HyunHyunLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = '릴리스 노트를 불러오는 데에 실패했습니다.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `새로운 ${isPrerelease(data.version) ? '사전 릴리스' : '릴리스'} 사용 가능`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('GitHub에서 다운로드<span style="font-size: 10px;color: gray;text-shadow: none !important;">런처를 닫고 dmg 파일을 실행하여 업데이트합니다.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = '최신 버전입니다.'
